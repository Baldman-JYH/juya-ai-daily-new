...
MIGRATION_NOTICE = "由于原 GitHub 账号被封禁，本仓库为新的发布地址；旧仓库、旧 Issues 链接和历史订阅地址已无法恢复。"
RSS_SERVICE_NOTICE = "RSS正常提供服务中。Folo 可能出现接收不到最新文章以及接收文章时间有较大延迟的问题，建议使用其他RSS阅读器"
RSS_DESCRIPTION = "资讯内容由AI辅助生成，可能存在错误，请以原始信息出处和官方信息为准。RSS正常提供服务中，但Folo可能出现接收不到最新文章以及接收文章延迟的问题。建议使用其他RSS阅读器。"
...
def get_pages_base_url(repo_name):
    custom_base_url = os.environ.get("SITE_BASE_URL")
    if custom_base_url:
...
def get_repo_pages_feed_url(repo, feed_filename):
    return f"{get_pages_base_url(repo.full_name)}/{feed_filename}"


def get_repo_pages_issue_url(repo, issue_number):
    return f"{get_pages_base_url(repo.full_name)}/issue-{issue_number}/"
...
def generate_rss_feed(repo, filename, me):
    ...
    generator.description(RSS_DESCRIPTION)
